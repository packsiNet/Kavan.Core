        ...
        public async Task<IReadOnlyList<SignalResultDto>> AnalyzeAsync(SignalRequestDto request, CancellationToken cancellationToken)
        {
            ...
            foreach (var symbol in symbols)
            {
                foreach (var tf in timeframes)
                {
                    ...
                    if (request.Filters?.ApplyBefore == true && !await PassesFiltersAsync(symbol, tf, request.Filters!, cancellationToken))
                    {
                        results.Add(new SignalResultDto
                        {
                            ...
                            Confidence = 0,
                            Explanation = "Rejected by pre-filters",
                            MatchedConditions = new List<string>()
                        });
                        continue;
                    }

                    // Evaluate conditions (flat and grouped)
                    var evals = new List<ConditionEvaluationResult>();

                    foreach (var cond in request.Conditions)
                        evals.Add(await EvaluateConditionAsync(symbol, tf, cond, cancellationToken));

                    foreach (var group in request.Groups)
                        evals.Add(await EvaluateGroupAsync(symbol, tf, group, cancellationToken));

                    // Aggregate scoring
                    var matchedEvals = evals.Where(e => e.Matched).ToList();
                    var score = matchedEvals.Sum(e => Math.Max(0, e.ScoreContribution)) * 10; // normalize to 0-100 range
                    score = Math.Clamp(score, 0, 100);

                    // Post-filters possibly penalize
                    if (request.Filters?.ApplyBefore == false && !await PassesFiltersAsync(symbol, tf, request.Filters!, cancellationToken))
                    {
                        score = Math.Max(0, score - 20 * _thresholds.FilterPenaltyWeight);
                    }

                    var strength = SignalStrength.FromScore(score, _thresholds.WeakMax, _thresholds.MediumMax);
                    var explanation = BuildExplanation(matchedEvals);

                    // Simple heuristic: if more confirmations are bullish, emit Buy; otherwise Sell/None
                    var signalType = InferSignalType(matchedEvals);

                    // Aggregate matched types, detailed types and attributes
                    var matchedTypes = matchedEvals.Select(e => e.Type).Where(t => !string.IsNullOrWhiteSpace(t)).Distinct().ToList();
                    var matchedDetailedTypes = ExtractDetailedTypes(matchedEvals);
                    var attributes = MergeAttributes(matchedEvals);
                    ...
                    results.Add(new SignalResultDto
                    {
                        ...
                        MatchedConditions = matchedEvals.SelectMany(e => e.MatchedConditions).Distinct().ToList(),
                        MatchedTypes = matchedTypes,
                        MatchedDetailedTypes = matchedDetailedTypes,
