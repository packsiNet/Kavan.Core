    ...
    IRepository<Candle_1d> c1d) : IConditionEvaluator
{
    ...
    private async Task<List<CandleBase>> GetRecentCandlesAsync(string symbol, string timeframe, int take, CancellationToken ct)
    {
        ...
        {
            "1m" => c1m.Query().Where(c => c.CryptocurrencyId == cryptoId).OrderByDescending(c => c.CloseTime),
            "5m" => c5m.Query().Where(c => c.CryptocurrencyId == cryptoId).OrderByDescending(c => c.CloseTime),
            "1h" => c1h.Query().Where(c => c.CryptocurrencyId == cryptoId).OrderByDescending(c => c.CloseTime),
            "4h" => c4h.Query().Where(c => c.CryptocurrencyId == cryptoId).OrderByDescending(c => c.CloseTime),
            "1d" => c1d.Query().Where(c => c.CryptocurrencyId == cryptoId).OrderByDescending(c => c.CloseTime),
            _ => c1h.Query().Where(c => c.CryptocurrencyId == cryptoId).OrderByDescending(c => c.CloseTime)
        };
