...
[InjectAsScoped]
public class IchimokuRedKumoService(ApplicationDbContext db, ISignalLoggingService logger) : IIchimokuRedKumoService
{
    ...
    public async Task<List<BreakoutResult>> DetectRedKumoAsync(List<string> symbols, List<string> timeframes, int lookbackPeriod)
    {
        ...
        foreach (var symbol in symbolList)
        {
            ...
            foreach (var tf in tfList)
            {
                ...
                var valid = red && (priceBelow || (priceInside && thickness >= atr * 0.3m && (avgVol == 0 ? false : (last.Volume / avgVol) >= 1m)));
