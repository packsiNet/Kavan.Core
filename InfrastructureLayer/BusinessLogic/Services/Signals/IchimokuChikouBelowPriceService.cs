...
[InjectAsScoped]
public class IchimokuChikouBelowPriceService(ApplicationDbContext db, ISignalLoggingService logger) : IIchimokuChikouBelowPriceService
{
    ...
    public async Task<List<BreakoutResult>> DetectChikouBelowPriceAsync(List<string> symbols, List<string> timeframes, int lookbackPeriod)
    {
        ...
        foreach (var symbol in symbolList)
        {
            ...
            foreach (var tf in tfList)
            {
                ...
                var pastIdx = candlesAsc.Count - 27;
                var past = candlesAsc[pastIdx];
                var pastPrev = candlesAsc[pastIdx - 1 < 0 ? 0 : pastIdx - 1];

                var chBelow = last.Close < past.Close - Math.Max(tol, atr * 0.1m);
                var prevChBelow = prev.Close < pastPrev.Close;
                var crossDown = !prevChBelow && chBelow;
                ...
                if (valid)
                {
                    ...
                    await _logger.LogSignalAsync(
                        ...
                        timeframe: tf,
                        signalName: "IchimokuChikouBelowPrice",
                        direction: -1,
                        breakoutLevel: past.Close,
