...
[InjectAsScoped]
public class IchimokuGreenKumoService(ApplicationDbContext db, ISignalLoggingService logger) : IIchimokuGreenKumoService
{
    ...
    public async Task<List<BreakoutResult>> DetectGreenKumoAsync(List<string> symbols, List<string> timeframes, int lookbackPeriod)
    {
        ...
        foreach (var symbol in symbolList)
        {
            ...
            foreach (var tf in tfList)
            {
                ...
                var green = cloud.A > cloud.B;
                var thickness = Math.Abs(cloud.A - cloud.B);
                var priceAbove = last.Close > top + Math.Max(tol, atr * 0.1m);
                var priceInside = last.Close <= top && last.Close >= bottom;
                var priceBelow = last.Close < bottom - Math.Max(tol, atr * 0.1m);

                var valid = green && (priceAbove || (priceInside && thickness >= atr * 0.3m && (avgVol == 0 ? false : (last.Volume / avgVol) >= 1m)));
