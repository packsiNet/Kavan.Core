...
[InjectAsScoped]
public class IchimokuChikouAbovePriceService(ApplicationDbContext db, ISignalLoggingService logger) : IIchimokuChikouAbovePriceService
{
    ...
    private static (decimal A, decimal B) ComputeCloudAtIndex(List<CandleBase> candlesAsc, int endIndex)
    {
        if (endIndex < 52) return (0m, 0m);
        var slice9 = candlesAsc.Take(endIndex + 1).TakeLast(9);
        var slice26 = candlesAsc.Take(endIndex + 1).TakeLast(26);
        var slice52 = candlesAsc.Take(endIndex + 1).TakeLast(52);
        var tenkan = Mid(slice9);
        var kijun = Mid(slice26);
        var spanA = (tenkan + kijun) / 2m;
        var spanB = Mid(slice52);
        return (spanA, spanB);
    }
    ...
    public async Task<List<BreakoutResult>> DetectChikouAbovePriceAsync(List<string> symbols, List<string> timeframes, int lookbackPeriod)
    {
        ...
        foreach (var symbol in symbolList)
        {
            ...
            foreach (var tf in tfList)
            {
                ...
                var pastIdx = candlesAsc.Count - 27; // index of candle 26 back
                var past = candlesAsc[pastIdx];
                var pastPrev = candlesAsc[pastIdx - 1 < 0 ? 0 : pastIdx - 1];

                var chAbove = last.Close > past.Close + Math.Max(tol, atr * 0.1m);
                var prevChAbove = prev.Close > pastPrev.Close;
                var crossUp = !prevChAbove && chAbove;

                var windowStart = Math.Max(pastIdx - 2, 0);
                var window = candlesAsc.Skip(windowStart).Take(5).ToList();
                var pastHighWindow = window.Max(c => c.High);
                var pastLowWindow = window.Min(c => c.Low);
                var free = last.Close > pastHighWindow + Math.Max(tol, atr * 0.1m);

                var cloudNow = ComputeCloud(candlesAsc);
                var top = Math.Max(cloudNow.A, cloudNow.B);
                var bottom = Math.Min(cloudNow.A, cloudNow.B);
                var priceAboveCloud = last.Close > top + Math.Max(tol, atr * 0.1m);
                var priceInsideCloud = last.Close <= top && last.Close >= bottom;

                var cloudPast = ComputeCloudAtIndex(candlesAsc, pastIdx);
                var chikouAbovePastCloud = last.Close > Math.Max(cloudPast.A, cloudPast.B);

                var gapAbs = Math.Abs(last.Close - past.Close);
                var gapOk = gapAbs >= atr * 0.1m;
                var volOk = avgVol == 0 ? false : (last.Volume / avgVol) >= 1m;

                var valid = chAbove && (crossUp || free) && gapOk && (
                    priceAboveCloud || (priceInsideCloud && volOk) || chikouAbovePastCloud
                );
                ...
                if (valid)
                {
                    ...
                    await _logger.LogSignalAsync(
                        ...
                        timeframe: tf,
                        signalName: "IchimokuChikouAbovePrice",
                        direction: 1,
                        breakoutLevel: past.Close,
