...
[InjectAsScoped]
public class TenkanKijunBullishCrossService(ApplicationDbContext db, ISignalLoggingService logger) : ITenkanKijunBullishCrossService
{
    ...
    public async Task<List<BreakoutResult>> DetectBullishCrossAsync(List<string> symbols, List<string> timeframes, int lookbackPeriod)
    {
        ...
        foreach (var symbol in symbolList)
        {
            ...
            foreach (var tf in tfList)
            {
                ...
                var priceAboveKijun = last.Close >= tk.Kijun + Math.Max(tol, atr * 0.1m);
                var slopeTenkan = tk.Tenkan - tk.PrevTenkan;
                var slopeKijun = tk.Kijun - tk.PrevKijun;
                var positionAbove = last.Close > top;
                ...
                var wickOk = (last.High - last.Close) <= (atr * 0.6m);

                var valid = cross && priceAboveKijun && chikouOk && (
                    positionAbove || (positionInside && slopeTenkan > 0 && (avgVol == 0 ? false : (last.Volume / avgVol) >= 1m))
                ...
                if (valid)
                {
                    ...
                    await _logger.LogSignalAsync(
                        ...
                        timeframe: tf,
                        signalName: "IchimokuTenkanKijunBullishCross",
                        direction: 1,
                        breakoutLevel: tk.Kijun,
