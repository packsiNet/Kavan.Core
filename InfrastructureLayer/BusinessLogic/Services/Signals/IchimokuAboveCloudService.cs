...
[InjectAsScoped]
public class IchimokuAboveCloudService(ApplicationDbContext db, ISignalLoggingService logger) : IIchimokuAboveCloudService
{
    ...
    public async Task<List<BreakoutResult>> DetectPriceAboveCloudAsync(List<string> symbols, List<string> timeframes, int lookbackPeriod)
    {
        ...
        foreach (var symbol in symbolList)
        {
            ...
            {
                var lookback = Math.Max(lookbackPeriod > 0 ? lookbackPeriod : 120, 60); // need at least 52
                ...
                var above = last.Close >= top + Math.Max(tol, atr * 0.2m);

                if (above && bodyOk && wickOk)
                {
                    ...
                    await _logger.LogSignalAsync(
                        ...
                        timeframe: tf,
                        signalName: "IchimokuPriceAboveCloud",
                        direction: 1,
                        breakoutLevel: top,
