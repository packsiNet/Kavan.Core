    ...
    public async Task<int> SyncLatestAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var row in dto.result.rows)
        {
            ...
            if (existing != null)
            {
                ...
                inserted++;
                _logger.LogInformation("Dune ETF issuer flow updated: {Ticker} {Time}", existing.EtfTicker, existing.Time);
                continue;
            }
            ...
            await _uow.SaveChangesAsync(cancellationToken);
            inserted++;
            _logger.LogInformation("Dune ETF issuer flow inserted: {Ticker} {Time}", entity.EtfTicker, entity.Time);
        }
