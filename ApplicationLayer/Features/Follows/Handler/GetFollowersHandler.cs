...
public class GetFollowersHandler(IRepository<UserFollow> followRepo, IRepository<UserAccount> userRepo, IRepository<UserProfile> profileRepo)
    : IRequestHandler<GetFollowersQuery, HandlerResult>
{
    public async Task<HandlerResult> Handle(GetFollowersQuery request, CancellationToken cancellationToken)
    {
        if (request.UserId <= 0)
        ...
        var query = from f in followRepo.Query()
                    join u in userRepo.Query() on f.FollowerUserId equals u.Id
                    join p in profileRepo.Query() on u.Id equals p.UserAccountId into up
                    from p in up.DefaultIfEmpty()
                    where f.FolloweeUserId == request.UserId
